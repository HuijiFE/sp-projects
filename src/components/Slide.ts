...
export default class Slide {
  ...
  public constructor(el: string | HTMLElement) {
    ...
    if (contentWrapper) {
      ...
      this.contents = Array.from(dom.querySelectorAll('.ylzt-slide_content'));
    }

    this.indicatorPrevious = dom.querySelector('.ylzt-slide_indicator-previous');
    if (this.indicatorPrevious) {
      this.indicatorPrevious.addEventListener('click', () => this.scroll(-1));
    }

    this.indicatorNext = dom.querySelector('.ylzt-slide_indicator-next');
