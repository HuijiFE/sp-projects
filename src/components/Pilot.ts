...
export default class Pilot {
  ...
  public constructor(el: string | HTMLElement) {
    ...
    this.images = Array.from(dom.querySelectorAll('.ylzt-pilot_'));
    // this.labels = Array.from(dom.querySelectorAll('.ylzt-pilot_label'));
    this.icons = Array.from(dom.querySelectorAll('.ylzt-pilot_icon'));
  ...
  private select(targetIndex: number): void {
    ...
    this.images.forEach((image, index) => {
      ...
      anime({
        ...
        easing,
        begin: () =>
          (this.icons[index].className = `ylzt-pilot_icon ${
